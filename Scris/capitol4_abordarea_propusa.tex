...
\begin{enumerate}
  \item Agentul de colectare (\textit{agent}) rulează pe fiecare gazdă protejată și
  ...
  \item Serverul central de analiză și decizie (\textit{central server}) reprezintă
  ...
  \item Containerul honeypot (\textit{hp-shell}) este mecanismul de izolare și
  ...
  \item Panoul de control (\textit{dashboard}) oferă operatorilor de securitate o
...
Fluxul de date parcurge sistemul în patru faze (Figura~\ref{fig:cap4-flux}):
...
\begin{figure}[h]
  ...
  \begin{tikzpicture}[
      ...
      box/.style={
        ...
        inner sep=4pt},
      agent/.style={box, fill=cap4agent},
      bus/.style={box, fill=cap4bus},
      server/.style={box, fill=cap4server},
      hp/.style={box, fill=cap4hp},
      flow/.style={-{Stealth[length=2.2mm]}, line width=0.5pt, gray!70!black},
      lbl/.style={font=\scriptsize\itshape, text=black!65},
    ]
    \definecolor{cap4agent}{RGB}{225,245,238}
    \definecolor{cap4bus}{RGB}{241,239,232}
    \definecolor{cap4server}{RGB}{238,237,254}
    \definecolor{cap4hp}{RGB}{250,236,231}

    \node[agent]  (agent)  at (0,0)      {\textbf{Agent de colectare}\\[1pt]\scriptsize eBPF $\cdot$ Tracee};
    \node[bus]    (redis)  at (4.6,0)    {\textbf{Redis Streams}\\[1pt]\scriptsize evenimente $\cdot$ comenzi};
    \node[server] (server) at (9.2,0)    {\textbf{Server central}\\[1pt]\scriptsize Isolation Forest};
    \node[hp]     (hp)     at (0,-2.6)   {\textbf{Honeypot}\\[1pt]\scriptsize container Docker};

    \draw[flow] (agent.north east) -- node[lbl, above] {evenimente} (redis.north west);
    \draw[flow] (redis.north east) -- (server.north west);
    \draw[flow] (server.south west) -- node[lbl, below] {comenzi} (redis.south east);
    \draw[flow] (redis.south west) -- (agent.south east);
    \draw[flow] (agent.south) -- node[lbl, right] {redirectare sesiune SSH} (hp.north);
  \end{tikzpicture}
  \caption[Fluxul de date în KernelTrap]{Fluxul de date în KernelTrap. Cele patru componente comunică exclusiv prin Redis
  Streams: agentul publică evenimente de syscall, serverul central le scorează și emite comenzi
  de pivotare, iar agentul redirectează sesiunea SSH compromisă către containerul honeypot.}
  \label{fig:cap4-flux}
\end{figure}
...
Isolation Forest returnează pentru fiecare eveniment un scor continuu prin\linebreak
...
Spre deosebire de jurnalele clasice, care rețin doar comenzile executate,
înregistrarea integrală a terminalului păstrează și ieșirile programelor, precum
și editările interactive de fișiere, oferind o reconstituire fidelă a sesiunii
așa cum a fost ea percepută de atacator. Această fidelitate este valoroasă mai
ales atunci când atacatorul folosește editoare în mod text sau interpretoare
interactive, ale căror acțiuni nu lasă urme în istoricul shell-ului.
...
Faptul că evenimentele containerului folosesc exact același format ca cele ale
agenților de pe gazdele reale înseamnă că serverul central nu are nevoie de o cale
de tratare separată pentru sursa lor: aceleași etape de scoring și agregare se
aplică uniform, indiferent dacă evenimentul provine de pe o gazdă protejată sau
din interiorul honeypot-ului. Astfel, un atacator care continuă să execute comenzi
de recon sau de escaladare după pivotare rămâne sub observație, iar comportamentul
său din mediul de confinement poate fi comparat direct cu cel dinaintea pivotării.
