                ...
                for _stream, messages in result:
                    for msg_id, fields in messages:
                        ...
                        if cmd.get("action") == "pivot":
                            _trigger_pivot(cmd.get("uid"), cmd.get("user", ""), self._pivot_script)
            except Exception as e:
                print(f"[CommandListener] error: {e} — retrying in 1s", file=sys.stderr)
                time.sleep(1)


def _trigger_pivot(uid: Optional[int], username_hint: str, pivot_script: str):
    """Execute the honeypot pivot script for the given user.

    Resolves the UID to a username on THIS host (the agent's /etc/passwd is
    authoritative — the central server may have a different UID→name mapping).
    Falls back to ``username_hint`` from the central server only if uid is
    missing (legacy commands) or local resolution fails.
    """
    username = ""
    if uid is not None:
        try:
            ...
            print(f"[PIVOT] Cannot resolve uid {uid} on this host; falling back to hint '{username_hint}'", file=sys.stderr)
    if not username:
        username = username_hint
    if not username:
        print(f"[PIVOT] No username available (uid={uid}, hint='{username_hint}') — skipping", file=sys.stderr)
        return
    print(f"[PIVOT] Pivoting user '{username}' (uid={uid}) to honeypot", file=sys.stderr)
    try:
...
class SyscallLogger:
    ...
    def __init__(
        ...
        # Re-read `who` on a short interval so a freshly logged-in SSH user is
        # picked up quickly. Was 30s — too slow: a user who logged in and
        # immediately ran a loud tool (e.g. linpeas) had their first ~30s of
        # events dropped before the session registered, starving the pivot count.
        self._ssh_refresh_interval: float = 3.0
        # Floor between lazy (on-demand) refreshes triggered by an event from an
        # as-yet-unregistered real user — bounds the `who` subprocess rate.
        self._ssh_lazy_min_interval: float = 1.0
        print(f"[Agent] SSH session UIDs at startup: {self._ssh_uids or 'none'}", file=sys.stderr)

        # Drop events generated by the agent itself or its direct parent.
        # Without this, our own python3 ↔ Redis socket activity is forwarded
        # back through the pipeline and immediately re-scored as anomalous —
        # observed ~20 sev-2 events per login from python3 net_tcp_connect.
    ...
    def _process_event(self, event: SyscallEvent, csv_writer: Optional[CSVWriter]):
        ...
        # Drop events generated by the agent itself or its direct parent
        # (the bash wrapper from start_agent.sh that sudo-launched us).
        if event.processId == self._own_pid or event.parentProcessId == self._own_pid:
            return
        ...
        if event.userId not in self._ssh_uids:
            # A new SSH session's UID isn't in the cached set until the next
            # interval refresh. Before dropping a real user's (uid >= 1000)
            # event, do a rate-limited immediate re-read of `who` so the session
            # registers on its FIRST events instead of losing the loud opening
            # of a tool like linpeas.
            if event.userId >= 1000:
                now = time.monotonic()
                if now - self._ssh_last_refresh >= self._ssh_lazy_min_interval:
                    self._ssh_uids = _get_ssh_uids()
                    self._ssh_last_refresh = now
            if event.userId not in self._ssh_uids:
                return
